...
The handler consists of a memory bank representing the available memory pool and
the page map containing the states for all pages in the memory bank.


> Memory Bank

The memory bank is the central pool of all usable memory. It can either be
provided by the user or be allocated dynamically using the configured functions.
The memory is divided into pages with the size of each page set through the
PG_PAGE_SIZE macro.
